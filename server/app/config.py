...
class Settings(BaseSettings):
    ...
    **기본값이 비활성인 이유:** 향상은 공짜가 아니다. 이미 깨끗한 오디오에
    ...
    antispoof_threshold: float = 0.999
    """spoof 확률 판정 임계값. 이 값 이상이면 차단한다.

    ⚠ **이 값은 반드시 배포 환경의 실제 오디오로 재캘리브레이션해야 한다.**

    AASIST-L은 ASVspoof 2019 LA로 학습됐고, 그 bonafide는 VCTK 코퍼스다. **다른
    녹음 조건의 진짜 음성을 위조로 오인한다** — 구간별 오탐률 실측:

        ASVspoof bonafide (도메인 안)   1.0%
        LibriSpeech       (도메인 밖)  13.3%   ← 13배

    여기에 구간별 최댓값 집계가 곱해진다. 구간별 오탐률 p, 구간 n개면 오탐은
    1-(1-p)^n로 커진다. LibriSpeech 20초(5구간)에서 **임계값 0.7이 진짜 사람
    음성의 75%를 차단**했다.

    도메인 밖(LibriSpeech 10초) 실측 운영 지점:
    ...
    0.999를 기본값으로 택한 이유: **오탐이 서비스를 망가뜨린다.** 정상 사용자를
    차단하면 인증 자체가 쓸 수 없게 되는 반면, 이 지점에서도 위조 탐지 99%는
    유지된다. 도메인 안에서라면 훨씬 낮은 값이 맞지만(0.7에서 탐지 99.9%),
    **어느 도메인인지 모르는 배포에는 보수적 값이 안전하다.**

    캘리브레이션 방법: `python -m eval.antispoof_length_eval`로 자사 오디오의
    길이별 오탐을 재고, 정상 차단 1% 지점을 임계값으로 삼는다.
    """
