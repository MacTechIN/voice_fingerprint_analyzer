...
> **참고:** 본 설계는 CSS 표준 방식에 따른 것이다. 장시간 오디오를 조각으로 나눠 선처리·재사용하는 바깥 계층 패턴(SEP v2 / LEP v2)은 [chunked-preprocess-pattern.md](chunked-preprocess-pattern.md)에 보관돼 있으며, 시간 스케일과 목적이 달라 본 절의 설계와 상호 배타적이지 않다. 활용 시점은 그 문서 머리말 참조.
